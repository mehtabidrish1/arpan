...
import 'package:arpan/models/training_surveyQuestionOptionsModel.dart';
...
class TrainingSurveyQuestionOptionsApi {
  Future<ResponseModel> getTrainingSurveyQuestionOptions(
      {String? surveyId, int? pageNumber, int? pageSize}) async {
    DynamicRequest dynamicRequest = DynamicRequest();

    String path = ApiConstants.trainingSurveyQuestionOptionsPath
    ...
    if (data is Success) {
      TrainingSurveyQuestionOptionsModel trainingSurveyQuestionOptionsModel =
          dynamicRequest.deserialize(
              data.response!, 'TrainingSurveyQuestionOptionsModel');
      return ResponseModel(
          code: data.code,
          response: trainingSurveyQuestionOptionsModel,
          isSuccess: true);
    }
    if (data is Failure) {
      return ResponseModel(
